class Game {
  ...
  play(){
    ...
    if(player.distance > 4000 && passedFinish === false){
      Player.updateFinishedPlayers();
      player.playerRank=finishedPlayers;
      player.update();
      passedFinish=true;
  ...
  displayRank(){
    ...
    image(silverIMG, displayWidth/4, displayHeight/10, 225, 270);
    ...
    for(var plr in allPlayers){
      ...
      if(allPlayers[plr].playerRank===1){
        text("First: "+ allPlayers[plr].name, 0, 85);
      }
      else if(allPlayers[plr].playerRank===2){
        text("Second: "+ allPlayers[plr].name, displayWidth/4, displayHeight/9+125);
      }
      else if(allPlayers[plr]. playerRank===3){
